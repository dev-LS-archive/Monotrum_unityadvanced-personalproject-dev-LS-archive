...
SIL Open Font License로 배포가 허용되었기에 라이선스 파일을 포함하여 커밋 가능
...
무채색이 많은 배경에 화려한 색상의 툰셰이딩 캐릭터가 대비를 이루고 캐릭터의 외견 상 음악을 들으며 달리는 모습에 어울린다.
...
![alt text](Resources/UnityChanSSU.png)
  ...
  - 그리고 우선 해당 캐릭터를 가져오기전 작업할 프로젝트 파일에 툰쉐이더와 캐릭터의 본을 살랑거리게 움직일 Unity Chan Spring Bone을 설치한다
    - `com.unity.toonshader`을 통해 패키지 파일을 설치한다.
      - 매뉴얼 링크: https://docs.unity3d.com/Packages/com.unity.toonshader@0.13/manual/installation.html
    - Unity Chan Spring Bone을 설치한다
      - https://github.com/unity3d-jp/UnityChanSpringBone.git#1.2.1-preview
  - 캐릭터만 패키지 파일로 만들어 가져온다.
